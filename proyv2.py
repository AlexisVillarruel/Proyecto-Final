...
with open('LaAlamedavfinal.json') as f:
...
class MainWindow(QMainWindow):
    ...
    def calcular_p_median(self):
        ...
        def distancia_total(ubicacion, puntos_demandas, G):
            return sum(ox.distance.euclidean_dist_vec(ubicacion[0], ubicacion[1], punto[0], punto[1]) for punto in puntos_demandas)
    ...
    def calcular_centroide_ponderado(self):
        ...
        centroides = [np.mean(feature['geometry']['coordinates'][0], axis=0) for feature in poligonos]
        poblaciones = [feature['properties'].get('population', 1) for feature in poligonos]
        ...
        ubicacion_centroide_ponderado = ox.nearest_nodes(G, centroide_ponderado[0], centroide_ponderado[1])
